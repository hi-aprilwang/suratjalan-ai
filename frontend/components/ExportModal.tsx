...
export const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, report }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent onClose={onClose} className="max-w-2xl bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 p-0 overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 flex items-center justify-between">
        ...
        <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div>
            <div className="text-sm uppercase tracking-wider text-zinc-600 dark:text-zinc-400 mb-2 font-semibold">
                ...
                <button
                  ...
                  className={`p-3 rounded-lg border text-left transition-all ${
                    selectedTarget === sys.id
                      ? 'bg-zinc-100 dark:bg-zinc-900 border-zinc-400 dark:border-zinc-500 shadow-sm ring-1 ring-zinc-400/40 dark:ring-zinc-500/40 text-zinc-900 dark:text-zinc-100'
                      : 'bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:border-zinc-300 dark:hover:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-900/40'
                  }`}
                >
                  <span className="font-bold text-sm block text-zinc-900 dark:text-zinc-100">{sys.name}</span>
                  <span className="text-sm text-zinc-500 block mt-1">{sys.desc}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Sync Result Banner */}
          {syncedResult && (
            <div className="bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-300 dark:border-emerald-800/80 rounded-lg p-3 flex items-center gap-2.5 text-emerald-800 dark:text-emerald-300 text-sm font-semibold">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm uppercase tracking-wider text-zinc-600 dark:text-zinc-400 font-semibold">
              ...
              <Button
                ...
                size="sm"
                onClick={handleCopy}
                className="h-7 text-sm font-medium gap-1.5 border-zinc-300 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300"
              >
            ...
            <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-900 overflow-hidden text-sm">
              <pre className="p-3.5 text-zinc-200 max-h-60 overflow-y-auto leading-relaxed">
                {payloadString}
              </pre>
            </div>
          </div>
        ...
        {/* Footer Actions */}
        <div className="px-5 py-3.5 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 flex items-center justify-between gap-3">
          <Button
            variant="outline"
            size="sm"
            onClick={handleDownloadJSON}
            className="gap-2 font-medium text-sm border-zinc-300 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 h-9"
          >
          ...
          <Button
            ...
            disabled={isSyncing}
            className="gap-2 font-semibold text-sm h-9 bg-blue-600 hover:bg-blue-500 text-white"
          >
